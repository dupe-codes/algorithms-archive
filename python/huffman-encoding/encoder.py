...
class HuffmanNode:
    def __init__(
        ...
        right: typing.Optional["HuffmanNode"] = None,
    ) -> None:
...
class HuffmanEncoder:
    ...
    def _count_characters(self, input_file: str) -> None:
    ...
    def _build_encoding_tree(self) -> None:
        ...
        while len(heap) > 1:
            ...
            heapq.heappush(
                heap, HuffmanNode(None, left.count + right.count, left, right)
            )
    ...
    def _write_header(self, output: typing.BinaryIO) -> None:
        ...
        pseudo_eof_encoding = self._encoding_table.pop(HuffmanEncoder._PSEUDO_EOF)
        ...
        for ch, encoding in self._encoding_table.items():
            # >BI = big-endian unsigned char followed by unsigned int
            output.write(struct.pack(">BI", ord(ch), len(encoding)))

            # Each char of the encoding string is 0 or 1, so Python's string encode
            # method will write each as a single byte
            # Note: this is a bit wasteful
            output.write(encoding.encode())

        # Write the pseudo EOF character. Use -1 as a placeholder
        # >iB = big-endian signed int followed by unsigned char. Note that
        # we could use an unsigned int for the length instead
    ...
    def _write_encoded_data(
        self, input_file: typing.TextIO, output_file: typing.BinaryIO
    ) -> None:
        ...
        # Handle the case where the last chunk is not a full byte
        if len(chunk) > 0:
    ...
    def _fit_decoder(self, input_encoded_file: typing.BinaryIO) -> None:
        ...
        """
        num_items = struct.unpack(
            ">I", input_encoded_file.read(HuffmanEncoder._HEADER_LENGTH_NUM_BYTES)
        )[0]
        ...
        if eof_placeholder == -1:
            eof_encoding = input_encoded_file.read(eof_encoding_length).decode()
            self._encoding_table[eof_encoding] = HuffmanEncoder._PSEUDO_EOF

    def _write_decoded_data(
        self, input_file: typing.BinaryIO, output: typing.TextIO
    ) -> None:
        def read_bits(input_file: typing.BinaryIO) -> typing.Generator[str, None, None]:
            for byte in iter(lambda: input_file.read(1), b""):
                for bit in format(ord(byte), "08b"):
                    yield bit

        current_code = ""
        for bit in read_bits(input_file):
